...
const Ballpit: React.FC<BallpitProps> = ({ className = '', followCursor = true, ...props }) => {
  ...
  useEffect(() => {
    let ballpit: ReturnType<typeof createBallpit> | null = null;

    const timer = setTimeout(() => {
      const canvas = canvasRef.current;
      if (!canvas) return;

      // Make sure canvas has real dimensions before WebGL init
      const parent = canvas.parentElement;
      if (!parent || parent.offsetWidth === 0 || parent.offsetHeight === 0) return;

      ballpit = createBallpit(canvas, { followCursor, ...props });
